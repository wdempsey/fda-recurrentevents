...
% For (1), we discuss an important issue regarding measurement-error models when paired with recurrent event outcomes.
...
For the sake of notational simplicity, we leave the dependency of the conditional hazard function on~$H_{i,t}^{NX}$ implicit, and write~$h_i (t; \theta)$. \textcolor{blue}{In our current setting, we assume the health process $\{ x_i(s)\}_{0 \leq s < \tau_i}$ is directly observed.  Therefore, we can consider the event process~$\bfT_i$ conditional on the health process~$\bfx_i$, which results in the log-likelihood related to the event process being then given by}
\[
  L_n (\theta) = \sum_{i=1}^{n} \left ( \sum_{j=1}^{k_i}
    \log \left( h_i \left( T_{i,j}; \theta
      \right) \right) - H_{i} \left( \tau_i; \theta \right) \right)
\]
where~$H_{i} (\tau_i ; \theta) = \int_{0}^{\tau_i} h_{i} (t; \theta) dt$ is the cumulative hazard function. \textcolor{blue}{See Appendix~\ref{section:memproblems} for additional arguments in favor of our proposed approach.} Solving the associated score equations~$U_n (\theta) = {\bf 0}$ yields the maximum likelihood estimator~$\hat \theta$, where
\[
...
% \begin{rmk}[Design-based estimation]
\textcolor{blue}{
...
% With the necessary assumptions stated, we can now state Lemma~\ref{lemma:simpleasym}, which provides asymptotic theory.
\begin{lemma} \normalfont
...
\end{lemma}
\textcolor{blue}{Proof of Lemma~\ref{lemma:simpleasym} is presented in Appendix~\ref{proof:asymp}.} A design-unbiased estimator for~$\Xi(\theta)$ is
\begin{equation}
\label{eq:fisher}
...
\end{lemma}
\noindent \textcolor{blue}{Proof of Lemma~\ref{prop:optimal} is presented in Appendix~\ref{app:optimalweights}.}
...
\]
We set~$\Delta$ to mimic a 30-minute window for a $12$-hour day.  We set~$\theta_0 = \log(5/1000)$ to set a baseline risk of approximately 5 events per day. We consider two choices of~$\beta(s)$: (1) $\beta_0 + \exp(- \beta_1 s) $ \textcolor{blue}{which decays to 0 as $s$ approaches $\Delta$ from below}, and (2) $\beta_1  * \sin \left( 2 \pi \frac{s}{\Delta} - \pi/2 \right)$ \textcolor{blue}{which is significantly different from 0 as $s$ approaches $\Delta$ from below.}
...
% Extrapolating to a complete data analysis, we can see that the run time will continue to increase significantly with minimal improvement in the mean integrated squared error.
...
and~$t \in \mathbb{R}_+$. \textcolor{blue}{Note that~\cite{Petrovich2018} requires modeling the mean and covariance functions separately for $y=0$ and $y=1$.}
