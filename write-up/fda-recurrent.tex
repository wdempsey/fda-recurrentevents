...
A critical consideration for recent history functional linear models~\cite{KIM20111554} is the size of the bandwidth, $\Delta$.  While above we explore the bias-variance trade-off under misspecification of $\Delta$, here we develop a data-adaptive method for bandwidth selection.  Specifically, given the model complexity does not change as a function of $\Delta$, our proposal is to simple compare AIC across a range of bandwidths and choose that which maximizes the criterion.

Table~\ref{tab:aic_selection} presents AIC-based selection across the 500 simulated datasets analyzed in the prior section. We see markedly distinct behavior of the selection criterion in the two settings.  The AIC-based selection works well for the sinusoidal effect across subsampling rates, but performs poorly in the exponential setting. Recall that the sinusoidal effect (choice 2) is significantly different from 0 at $\Delta^\star$ and then 0 for all $s > \Delta^\star$ while the exponential effect (choice 1) decays slowly towards 0 as $s \to^{-} \Delta$. Therefore, the selection problem under the sinusoidal effect is much easier than under the exponential effect.  Moreover, bias, variance, and mean-integrated squared error are not substantially harmed by misspecification of the bandwidth in the exponential setting.  Thus, we conclude that the AIC-based selection method works well in settings where inference requires estimating the bandwidth accurately, as desired.
...
32 & 0.11 & 0.12 & 0.13 & 0.18 & {\bf 0.85} & {\bf 0.79} & {\bf 0.76} & {\bf 0.70} \\
35 & 0.13 & 0.13 & 0.14 & 0.15 & 0.05 & 0.06 & 0.06 & 0.09 \\
37 & 0.14 & 0.20 & 0.26 & {\bf 0.28} & 0.01 & 0.01 & 0.02 & 0.03 \\ \hline
\end{tabular}
\caption{Selection of bandwidth when true $\Delta = 32$ minutes.
% SINE EXAMPLE
}
...
Here we address the time-complexity of solving the approximate score equations relative to maximizing the log-likelihood using Lemma~\ref{lemma:logistic}.  The complexity of logistic regression with $n$ observations and covariates of dimension $d$ is $O(nd)$; so maximizing the log-likelihood is well approximated by subsampling with a very high rate so that the design-unbiased score functions have minimal approximation error in comparison to directly maximizing the likelihood function.  Under this assumption, at the final stage we have the relative complexity of $(4 \times 60 \times \tau_i) \cdot n \cdot d$ compared to the expected time-complexity under subsampling with rate $c$ per minute of $(c \times \tau_i) \cdot n \cdot d$.  In practice, this can lead to large differences in run time; specifically, this means sampling every minute rather than every half hour will be approximately $2^5$ times as long (from 5 minutes to 2.8 hours), while sampling every second will take $2^11$ times as long (from 5 minutes to 180 hours).  In our first example, the run time was 356 seconds (approximately 5.9 minutes) when sampling every five minutes.  Thus, if sampling increased to every minute or every second, then run time would increase to approximately 30 minutes and 30 hours respectively.  In our second example, the run time of 18 minutes would increase to 1.5 hours and 90 hours respectively.  In both instances, the relative efficiency gain would be negligible suggesting a huge computational increase for minimal relative information gain.
