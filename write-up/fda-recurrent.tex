...
\end{aligned}
$$
where the second and third line ignore the constant $(2\pi)^{q/2} |\Psi|^{-1/2}$. Differentiating the function $g(\theta, b_i, \pi_{ij}, \Psi)$ with respect to $b_i$ yields
$$
\begin{aligned}
\frac{\partial g(\theta, b_i, \pi_{ij}, \Psi)}{\partial b_i} &= \sum_{j=1}^{k_i} \left( y_{ij} Z_{ij} - \frac{Z_{ij}}{1+\exp(-W_{ij}^\top \theta - Z_{ij}^\top b_i - \log \pi_{ij}}\right) - \Psi^{-1} b_i \\
\frac{\partial g(\theta, b_i, \pi_{ij}, \Psi)}{\partial b_i^2} &= - \sum_{j=1}^{k_i} \frac{Z_{ij}Z_{ij}^\top}{(W_{ij}^\top \theta + Z_{ij}^\top b_i + \log \pi_{ij})^2} - \Psi^{-1}
\end{aligned}
$$
Redefining terms from the prior section, we can express the above as
$$
\begin{aligned}
\frac{\partial g(\theta, b_i, \pi_{ij}, \Psi)}{\partial b_i} &= Z_i E_i (\theta, b_i) - \Psi^{-1} b_i \\
\frac{\partial g(\theta, b_i, \pi_{ij}, \Psi)}{\partial b_i^2} &= - \left[ Z_i V_i (\theta, b_i) Z_i + \Psi^{-1} \right]
\end{aligned}
$$
...
$$
\hat b_i^{(k+1)} = \hat b_i^{(k)} + \left( \sum_{j=1}^{k_i} \frac{Z_{ij}Z_{ij}^\top}{(W_{ij}^\top \theta + Z_{ij}^\top \hat b_i^{(k)} + \log \pi_{ij})^2} + \Psi^{-1} \right)^{-1} \left( \sum_{j=1}^{k_i} \left( y_{ij} Z_{ij} - \frac{Z_{ij}}{W_{ij}^\top \theta + Z_{ij}^\top \hat b_i^{(k)} + \log \pi_{ij}}\right) - \Psi^{-1} \hat b_i^{(k)} \right)
$$
Fixing $\hat b_i$, we then need to estimate $\Psi$ and $\theta$. We employ adaptive gaussian quadrature (cites) as it has been shown that for infrequent events the Laplace approximation tends to underestimate effects (cites).
To this end, we

We write this as $\prod_{i=1}^n l_{ij} (\theta, b_i, \pi_{ij}) $
