...
The use of smart devices and other wearables to passively monitor patients has led to a rapid increase in the number of studies with high-frequency sensor streams that can be conceptualized as studies where measurements are functions.
The development of such applications have been accompanied by intense methodological development in regression models where some covariates are functions.  In this article, we devleop a novel inferential approach to functional regression~\citep{James2002,JamesSilverman2005,muller2005,Ramsay2005,kokoszka2017,doi:10.1198/jasa.2009.tm08564,doi:10.1198/016214507000000527}.

Recently, modeling time-to-event data with functional predictors has received a fair bit of attention.  Functional linear Cox regression model (FLCRM) considers the association between a time-to-event and a set of functional~$\{ X(s) \}$ and scalar predictors~$Z \in \mathbb{R}^p$~\citep{https://doi.org/10.1111/biom.12748}. In this setting, the functional and scalar predictors are observed at baseline and the hazard function depends on a non-parametric baseline, a functional linear model~$\int_S X(s) \beta(s)ds$ and a linear model for the baseline scalar predictors~$Z^\top \gamma$.
Since the linear assumption may be too restrictive,~\cite{doi:10.1080/10618600.2020.1853550} consider a more flexible additive functional Cox model for multivariate functional predictors in which the hazard depends on $\int  F \{s, h_{is}[X_i (s)]\} ds$ where $F$ is an unspecified bivariate twice differntiable function.

In our setting, the high frequency sensor process is an outcome and must therefore be jointly modelled with the time-to-event process.  Recently, \cite{Li2022} proposed a functional approach to joint modelling of multivariate longitudinal biomarkers and a time-to-event outcome~\cite{Tsiatis2004,Rizopoulos2010}.  Here, as in traditional joint models, the longitudinal biomarker is observed at only a few observation times (typically $<10$) and is modelled via a functional principal components analysis.   Shared  process  of  to identify the shared progression pattern and outcome-specific progression patterns of the outcomes.

Here a design-based perspective is taken which subsamples non-event times to  avoid some of the computationally demanding aspects of joint models
Subsampling in the context of functional data analysis for massive data has recently been investigated~\cite{https://doi.org/10.48550/arxiv.2104.03446} in which a functional L-optimality criterion is derived.


The use of historical functional linear models....

The main contribution of this paper is to combine advances in historical functional linear models with techniques from design-based inference to circumvent computationally challenging joint models.

The combination leads to a natural penalized generalized linear model that can be estimated with off-the-shelf software.
...
\subsection{Comparison to alternative methods} \label{section:simcomparisons}
