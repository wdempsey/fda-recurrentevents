...
\cite{ChenMuller2012}.  Our imputation procedure instead uses the
marginal covariance~$\Xi$ as a proxy for the true covariance
function~$\Sigma_t(\cdot, \cdot)$.  We found this to work well in
simulations as~$\Sigma_t$ was relatively constant across~$t$; however,
this imputation strategy may be inappropriate when the covariance
structure changes significantly with time~$t$.
...
circumvent some of the issues.  
In this section, we assume each individual is observed over five
12-hour days.  


First, we generate a sequence of event times via a marginal hazard
rate that ignores sensor data.  

We generate 5 days of 12 hours with range of marginal rates (1 per
12 hours, 1 per 6 hours, 1 per hour).  We then sample non-events at
rates or (1 per 12/6/1/0.5 hours).
