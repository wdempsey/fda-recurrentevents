...
\begin{itemize}
\item[${\bf T}_i$] \quad Observed event times on individual $i$.
\item[$\tau_i$] \quad Observation window-length for individual $i$
\item[$N_i(t)$] \quad Counting process associated with ${\bf T}_i$
\item[${\bf x}_i$] \quad Health process for individual $i$, i.e., $\{ x_i (s) \}_{0 < s <\tau_i} \}$
\item[$H_{i,t}^{NX}$] \quad The $\sigma$-field generated by all past values $(N_i(s), x_i(s))_{0 \leq s \leq t}$
\item[$h_i (t| H_{i,t}^{NX}; \theta)$] \quad Fully parametric conditional hazard function
\item[$h_0 (t; \gamma)$] \quad Parametrized baseline hazard function
\item[$\Delta$] \quad An unknown, window-length
\item[$\theta$] \quad All parameters underlying this hazard function including baseline parameters~$\gamma$, event-history and time-in-study parameters~$\alpha$, and the functional parameter $\{ \beta(s) \}_{0 \leq s \leq \Delta}$.
\item[$H_i (t; \theta)$] \quad Cumulative hazard function
\item[$U_n (\theta)$] \quad True score equations for log-likelihood of the event process
\item[${\bf D}_i$] \quad Sampled non-event times on individual $i$.
\item[$\hat H_i^{(1)} (t; \theta)$] \quad Waagepetersen estimator of the cumulative hazards using all $u \in {\bf T}_i \cup {\bf D}_{i}$
\item[$\hat U_n (\theta)$] \quad Approximate estimating equations leveraging the Waagepetersen estimator
\item[$\Sigma_y (s,s')$] \quad Marginal covariance over event or subsampling intensity $f_y (T)$
\item[$\mu_y (t,s)$] \quad Mean function $s$ time units before time $t$
\item[$\tilde U_n (\theta)$] \quad Approximate estimating equations leveraging both subsampling and functional data approximations
\end{itemize}
...
\section{Technical Arguments}

\subsection{Derivation of the design-unbiased score equations}
...
\subsection{Proof of Lemma~\ref{lemma:simpleasym}}
\label{proof:asymp}

\begin{newmaterial}
We break the proof of Lemma~\ref{lemma:simpleasym} into two sections.  First, we prove asymptotic consistency, i.e.,~$\hat \theta_n \to \theta_0$ in probability as $n \to \infty$ and $K_x \to \infty$.  Then, we prove asymptotic normality via Rebolledo's martingale central limit theorem.
\end{newmaterial}
\begin{proof}[Proof of asymptotic consistency]
...
\begin{equation}
\label{eq:decomp}
n^{-1} \tilde U_n (\theta_0) = \underbrace{n^{-1} U_n (\theta_0 )}_{(I)}  + \underbrace{n^{-1} \left(
  \hat U_n (\theta_0) - U_n (\theta) \right)}_{(II)} + \underbrace{n^{-1} \left(\tilde U_n
  (\theta_0) - \hat U_n (\theta_0) \right)}_{(III)}
\end{equation}
where, recall,~$\hat U_n (\theta_0)$ are the logistic score equations with $\int_0^\infty X_i(t,s) \beta(s) ds$ and~$\tilde U_n (\theta_0)$ with the approximation~$C_{i,t}^\top J_{\phi, \psi} {\bf b}$.
\textcolor{blue}{This decomposes the term into (I) the original score equations which we are known to yield asymptotically consistent estimates, (II) the difference between the original score equations and the design-unbiased score equations, and (III) the design-unbiased score equations with and without the approximation using the Karhunen-Loeve decomposition and~$K_x < \infty$.}
...
\end{align*}
where~$h(\cdot; \theta_0)$ is given by~\eqref{eq:hazardlinear}. Lenglart's inequality implies the second term converges in probability to zero under Assumptions~\ref{E1} and~\ref{E2}. Term (II) satisfies
\begin{align*}
...
expit function.
\textcolor{blue}{The first inequality is simply due to bringing the metric inside the summand. This allows us to upper bound the difference as a sum over differences in the truncated representation and the true representation.}  The second inequality is due to the Taylor remainder theorem \textcolor{blue}{applied to this difference. This allows us to upper bound the difference as a product of the impact of the non-linear transformation $g$, and the error due to truncation.} The third is due to~$g^\prime (x) = e^x/(1+e^x)^2 \leq e^0/(1+e^0)^2 = 1/4$, reordering the summation, and rewriting the error term in terms of the difference between~$X(t,s)$ and the approximation with truncation level~$K_x$. Letting~$n \to \infty$,  \textcolor{blue}{we can rewrite $n^{-1} \sum_{i=1} \sum_{t \in {\bf T \cup D}}$ as an integral with respect to the joint event and sampling distributions~$(f_0(t) + f_1 (t))$}. Finally, under Assumptions~\ref{assumption:truncation}, \cite{Park2018} show
\[
...
\multirow{3}{*}{26} & MISE & 0.070 & 0.075 & 0.095 & 0.0117 \\
 & Variance & $2.1 \times 10^{-2}$ & $2.7 \times 10^{-2}$ & $3.9 \times 10^{-2}$ & $5.8 \times 10^{-2}$ \\
  & P-MISE & 0.085 & 0.091 & 0.115 & 0.141 \\ \hline
\multirow{3}{*}{29} & MISE & 0.072 & 0.076 & 0.093 & 0.111 \\
 & Variance & $1.8 \times 10^{-2}$ & $2.3 \times 10^{-2}$ & $3.4 \times 10^{-2}$ & $5.0 \times 10^{-2}$ \\
  & P-MISE & 0.079 & 0.083 & 0.101 & 0.121 \\ \hline
\multirow{3}{*}{32} & MISE & 0.076 & 0.079 & 0.095 & 0.109 \\
 & Variance & $1.5 \times 10^{-2}$ & $2.0 \times 10^{-2}$ & $3.2 \times 10^{-2}$ & $4.6 \times 10^{-2}$ \\
  & P-MISE & 0.076 & 0.079 & 0.095 & 0.109 \\\hline
\multirow{3}{*}{35} & MISE & 0.083 & 0.085 & 0.099 & 0.114 \\
 & Variance & $1.4 \times 10^{-2}$ & $2.0 \times 10^{-2}$ & $3.2 \times 10^{-2}$ & $4.7 \times 10^{-2}$ \\
  & P-MISE & 0.081 & 0.083 & 0.096, & 0.110 \\\hline
\multirow{3}{*}{37} & MISE & 0.091 & 0.092 & 0.106 & 0.117 \\
 & Variance & $1.5 \times 10^{-2}$ & $2.4 \times 10^{-2}$ & $3.4 \times 10^{-2}$ & $5.0 \times 10^{-2}$ \\
  & P-MISE & 0.088 & 0.089 & 0.102 & 0.112 \\\hline
\end{tabular}
...
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=.8\linewidth]{../figures/smoothed_eda_nonevents.png}
  \caption{Electrodermal activity (EDA)}
  \label{fig:mean_eda_nonevents}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=.8\linewidth]{../figures/smoothed_ai_nonevents.png}
  \caption{Activity Index (AI)
}  \label{fig:mean_acc_nonevents}
\end{subfigure}
\caption{Mean EDA and AI at non-event times. Note: Due to the data size, non-event times for EDA were further sub-sampled to be maximum 1000 non-event times per individual which leads to the increased variation relative to AI plot}
\label{fig:mean_edacc_nonevents}
\end{figure}



\section{Code to Replicate Simulation and Case Study Results}
The R code used to generate the simulation experiments and case study results in this paper can be obtained at \verb"https://github.com/wdempsey/fda-recurrentevents/".
