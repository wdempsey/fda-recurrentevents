...
set.seed(1839713)
## CHOOSING 1000 random times for speed issues

#### Match button presses and the previous EDA (scaled) data ####
if(!file.exists("../summary_data/bp_eda_nonevents.rds")) {
  ...
  for (i in 1:n_obs){
    ...
    if (i%%10 == 0){
      print(i)
      saveRDS(bp_eda, file = '../summary_data/bp_eda_nonevents.rds')
    }
    ...
    if (file.exists(input_name) & i!=10 & i!=12){ 
      ...
      sub_nonevents_times = sample(sub_nonevents_times, min(1000, length(sub_nonevents_times)))
      eda = readRDS(input_name)
      device = unique(eda$Device)
      
      # process the data from different devices seperately due to the timestamp overlap 
      for (d in 1:length(device)){
        sub_eda =  filter(eda, Device==device[d])
        for (j in 1:length(sub_nonevents_times)){
          bp_time = sub_nonevents_times[j]
          # EDA_time in [BP_time-30min, BP_time)
          eda_per_device_bp = filter(sub_eda, timestamp<bp_time, timestamp>=(bp_time-30*60))
          if (dim(eda_per_device_bp)[1] > 0){
            sub_bp_eda = eda_per_device_bp %>%
  ...
  bp_eda = readRDS(file = '../summary_data/bp_eda_nonevents.rds')
}
...
png("C:/Users/Balthazar/Documents/GitHub/fda-recurrentevents//figures/smoothed_eda_nonevents.png",
