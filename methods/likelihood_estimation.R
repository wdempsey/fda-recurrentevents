...
## ACC
acc_event_model.matrix = readRDS("acc_event_modelmatrix_2021-12-20.RDS")
acc_nonevent_model.matrix = readRDS("acc_nonevent_modelmatrix_2021-12-20.RDS")
## EDA
eda_event_model.matrix = readRDS("eda_event_modelmatrix_2021-12-20.RDS")
eda_nonevent_model.matrix = readRDS("eda_nonevent_modelmatrix_2021-12-20.RDS")
...
Y = c(rep(1,nrow(acc_event_model.matrix)), rep(0, nrow(acc_nonevent_model.matrix)))

model.matrix = rbind(acc_event_model.matrix[,-c(1:3)], acc_nonevent_model.matrix[,-c(1:3)])
...
subsample_offset = rep(log_sampling_rate,nrow(model.matrix))
...
Sigma_direct = solve(t(X)%*%diag(W)%*%X)


betaHat.net_glm <- bb %*% temp_daytime$coefficients[-1]
...
plot(sequence[obs], betaHat.net_glm[obs], type= "l", 
     axes = FALSE, xlab = "Time until event", 
     ylab = expression(paste(beta, "(s)")))
axis(side = 1); axis(side = 2)
lines(sequence[obs], (betaHat.net_glm + 1.96 * stderr)[obs], col = "red")
lines(sequence[obs], (betaHat.net_glm - 1.96 * stderr)[obs], col = "red")
abline(h = 0, col = "blue")
dev.off()
