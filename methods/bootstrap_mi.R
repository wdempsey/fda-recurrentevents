...
for (Delta in set_of_Deltas) {
  for (type in set_of_types){
    ...
    event_means = readRDS(file = paste(type, "_lin_event_means_Delta_",Delta,"_2023-04-08.RDS", sep=""))
    event_times = readRDS(file = paste(type, "_lin_event_complete_case_timesincebaseline_Delta_",Delta,"_2023-04-08.RDS", sep=""))
    event_Sigma = readRDS(file = paste(type, "_event_Sigma_Delta_",Delta,"_2023-04-08.RDS", sep=""))
    event_complete = event_complete[,1:(3+ncol(event_Sigma))]
    ...
    for(current_mi in 1:num_imputes) {
      ...
      eig_values[eig_values< 0] = 0
      K = min(35,length(eig_Sigma_est$values))
      ...
      saveRDS(event_model.matrix, file = paste("./bootstrap_files_Delta/event_RDS/",type, "_event_modelmatrix_Delta_",Delta,"_bootstrap_",current_bootstrap,"_mi_",current_mi,"_",today(),".RDS", sep = ""))
...
for (Delta in set_of_Deltas) {
  for (type in set_of_types){
    print(paste("In non-event at type =",type))
    print(paste("And Delta =", Delta))
    nonevent_complete = readRDS(paste(type, "_nonevent_complete_Delta_",Delta,"_HLP_2023-04-06.RDS", sep=""))
    nonevent_means = readRDS(file = paste(type, "_lin_nonevent_means_Delta_",Delta,"_2023-04-08.RDS", sep=""))
    nonevent_times = readRDS(file = paste(type, "_lin_nonevent_complete_case_timesincebaseline_Delta_",Delta,"_2023-04-08.RDS", sep=""))
    nonevent_Sigma = readRDS(file = paste(type, "_nonevent_Sigma_Delta_",Delta,"_2023-04-08.RDS", sep=""))
    nonevent_complete = nonevent_complete[,1:(3+ncol(nonevent_Sigma))]
    ...
    for(current_mi in 1:num_imputes) {
      print(paste("Made it to non-event imputation number", current_mi))
      for(row in which(!full_obs)) {
      ...
      paste("Made it to the saveRDS part")
